/*
	...
	Name: Whitespell-CIO.js

	Copyright (c) 2014, Whitespell LLC
	All rights reserved.

	License: BSD-2-Clause

	Notes:
	Whitespell-CIO.js uses Socket IO (MIT License)
...
var WorkerHandler = function(options) {
    this.mem = C.addMemory('WorkerHandler');
    ...
    this.usingWorkers = (ie === false);

    if(this.options.apiKey) this.initWebSocket();
};

WorkerHandler.prototype.initWebSocket = function(){

    var self = this;

    //merge passed options with defaults
    var webSocketOptions = this.options.workers.WebSocket;

    this.assign("WebSocket", {
        task    :   "connect",
        params  : {
        ...
        //Listen for session(WebSocket)
        self.assign("WebSocket", {
            task: "sessionIdListener",
            params: {}
        }, function (data) {
            self.sessionId = data;
            self.assign("WebSocket", {
            ...
            // listen for variables that need to be updated
        ...
        self.assign("WebSocket", {
            ...
            if(wsUI !== undefined) {
               ...
               w$(data.varName, data.varContent);
            }
        });
    });
...
WorkerHandler.prototype.add = function (name) {
    ...
    try {
        ...
        if (window.BlobBuilder !== undefined) {
            ...
            //create new blob using blobbuilder
            var bb = new BlobBuilder();
            bb.append( this.workers[name] );
            blob = bb.getBlob('text/javascript');
            blob = window.URL.createObjectURL(blob);

        } else if (window.Blob) {
            //if blob is supported create blob
            blob = window.URL.createObjectURL( new Blob([ this.workers[name] ]) );
            this.workers[name] = true;
        }

        //create new worker using blob
        var w = new Worker(blob);

        //save worker in cache
        C.set(this.mem, 'Workers', name, w);
        ...
        try {
            ...
            //notify user that we're using a fallback
            console.log('falling back on WorkerSource');

            //make user using worker file
            var w = new Worker('../src/workers/' + name + '.js');
            ...
            //if worker using file generates error
            //fallback on using no workers
            console.log('Falling back on no workers');

            //parse worker file string using function constructor
            //store in global variable
            window[name] = new Function('return '+this.workers[name])();

            //set using workers to false
            this.usingWorkers = false;
    ...
    //if after all the above we're still using workers
    if (this.usingWorkers === true) {

        var self = this;

        //automaticly listen for response
...
WorkerHandler.prototype.handleResponse = function(event) {

    //if there is a apiKey and no sessionId
    if(typeof this.options.apiKey === 'string' && this.sessionId === 0 && typeof this.workers.WebSocket === 'string') {
        ...
        try {
            ...
            //set sessionId from server locally
            var sesId = jsonContent.sessionId;
            console.log(jsonContent);
            if(sesId !== undefined && sesId > 0){
                this.sessionId = jsonContent.sessionId;
            }

        } catch(e) {
            console.log('`'+event.content + '` is not valid JSON');
            console.log(e.message);
        }
    }
    var usedBlox = false;
    try {
        var p = C.get(this.mem, "WorkersCallbacks", event.task + "-" + event.uid).params.url;
        if(p !== null && p !== undefined && p.indexOf("/blox") !== -1) {
            usedBlox = true;
        }
    ...
    if(usedBlox) {
        ...
        try {
            var t = JSON.parse(event.content);
            if (t.updates !== undefined) {
                for (var update in t.updates) {
                    console.log("updated " + update +" to " + t.updates[update]);
                    w$(update, {
                        "value" : t.updates[update] // todo update lists
                    });
                }
            }
        }catch (e) {
            console.log("error parsing as json: " + e.message);
        }
    }

    //this.sessionId = event.data.
    C.get(this.mem, "WorkersCallbacks", event.task + "-" + event.uid).callback(event.content, C.get(this.mem, "WorkersCallbacks", event.task + "-" + event.uid).params);


};

WorkerHandler.prototype.getWorker = function (name) {
    if (name === undefined) {
        console.error('Invalid worker requested');
        return;
    }

    return C.get(this.mem, 'Workers', name);
};

WorkerHandler.prototype.mitigator = function(worker, sendData) {

    switch(worker) {

        case 'Cors' : {
            this.workers.Cors.CorsListen(sendData);
            break;
        }
        case 'WebSocket' : {
            this.workers.WebSocket.Listener(sendData);
            break;
        }
...
WorkerHandler.prototype.assign = function (workerName, data, callback) {

    //parse worker function when first used
    if( typeof this.workers[workerName] === 'string' ){
        this.add(workerName);
    }
    ...
    data.uid = Math.ceil((Math.random() * 1000)); // todo change unique algorithm


    if (data.cacheResult === true) {
        var pageContent = C.get(this.mem, 'PageCache', data.params.url);
        if (pageContent !== null) {
            setTimeout(function () {
            ...
            C.set(this.mem, 'PageCache', data.uid, data.params.url);
        }
    }

    C.set(this.mem, "WorkersCallbacks", data.task + "-" + data.uid, {"callback":callback, "params" : data.params});

    //if using worker Cors
    if(workerName === 'Cors') {

        //if apiKey is present
        if(typeof this.options.apiKey === 'string') {
            data.params.parseData.publicApiKey = this.options.apiKey;

            //if not using WebSocket
            if(typeof this.workers.WebSocket === 'string'){
                data.params.parseData.sessionId = this.sessionId;

                //if not using WebSockets and no sessionId
                if(this.sessionId === 0) {
                    data.params.parseData.deviceType = this.options.deviceType;
                    data.params.parseData.connectionType = 'WIFI';
                }

                //If using WebSocket and not already got sessionId
            } else if(typeof this.workers.WebSocket !== 'string' && this.sessionId !== 0) {
                data.params.parseData.sessionId = this.sessionId;
            }
        }

        //convert json to url query
        data.params.parseData = this.jsonToURL(data.params.parseData);
    ...
    if (this.usingWorkers === true) {
        ...
        this.mitigator(workerName, data);
    }

};

WorkerHandler.prototype.jsonToURL = function (obj) {
    var urlString = '';
    for (var key in obj) {
